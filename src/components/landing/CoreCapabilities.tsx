...
export default function CoreCapabilities() {
  ...
  return (
    <section ref={ref} className="py-24 md:py-32 px-6 md:px-8 bg-[#131313]">
      <div className="max-w-6xl mx-auto">
        <div
          className={`mb-12 border-b border-white/5 pb-6 transition-all duration-700 ${
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
          {features.map((f, i) => {
            ...
            return (
              <div
                key={f.title}
                className={`${f.span} group border border-white/10 bg-[#1b1b1b] p-8 flex flex-col justify-between hover:border-white/30 transition-colors duration-500 ${
                  visible
              ...
              >
                <div>
                  ...
                  <h3 className="text-xl md:text-2xl font-medium text-white mb-3 uppercase tracking-tight">
                    {f.title}
                  </h3>
                ...
                {f.tags && (
                  <div className="mt-8 flex gap-2">
                    {f.tags.map((tag) => (
                      <span
                        key={tag}
                        className="px-2 py-[2px] border border-white/10 font-mono text-[10px] text-white/50 tracking-wider"
                      >
                        {tag}
                      </span>
                    ))}
                ...
                {f.footer && (
                  <div className="mt-8 font-mono text-[10px] text-white/30 tracking-wider">
                    {f.footer}
                  </div>
                )}
              </div>
